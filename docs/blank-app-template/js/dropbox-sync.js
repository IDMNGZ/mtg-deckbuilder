...
var DropboxSync = (function () {
  ...
  // RENAME both of these to your own app - SAVE_PATH becomes the actual filename Dropbox
  // creates in the app's sandboxed folder, and the PKCE key prefix just needs to not
  // collide with any other app's sessionStorage keys (unlikely regardless, but cheap to
  // make explicit).
  var SAVE_PATH = "/yourapp-save.json";
  var TOKEN_REFRESH_SKEW_MS = 5 * 60 * 1000;
  var AUTO_PUSH_DEBOUNCE_MS = 3000;
  var PKCE_VERIFIER_KEY = "yourapp:dropbox:pkce-verifier";
  var PKCE_STATE_KEY = "yourapp:dropbox:pkce-state";
  var PKCE_REDIRECT_KEY = "yourapp:dropbox:pkce-redirect";
  ...
  function updateState(patch) {
    ...
    document.dispatchEvent(new CustomEvent("app:sync-status-changed"));
  }
  ...
  // connection backs up everyone sharing this device (see the System tab's "Switch
  // Profiles"), so the file needs to carry all of them for another device connecting the
  // same account to see every profile, not just whichever was active at the last push.
  ...
  // Verbose logging on every push/pull (see console) - sync bugs are rarely reproducible
  // from reading the code alone (see docs/SYNC-ARCHITECTURE-HANDOFF.md's whole "testing
  // methodology" section); having every step logged turns debugging a real report into
  // reading output instead of guessing. Worth keeping permanently, not just while chasing
  // a specific bug - the noise cost is low and it's exactly what you'll want the next time
  // something looks wrong.
  function summarizeProfiles(profiles) {
    return (profiles || []).map(function (p) {
      return p.name + " (" + p.id + "): " + Object.keys(p.items || {}).length + " items, " + (p.records || []).length + " records";
    });
  }
  ...
  function init() {
    ...
    document.addEventListener("app:data-changed", scheduleAutoPush);
