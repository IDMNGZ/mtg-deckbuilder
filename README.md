...
- The bookmark icon (next to "I own this," on a card tile or in its detail view) adds that specific printing to the **Wish List** tab, where each one links out to whatever vendors Scryfall has purchase links for (TCGplayer, Cardmarket, etc.) — plain convenience links, not affiliate links. Owning a card removes it from the Wish List automatically.
